...
async function getBookDetails(browser: Browser) {
    const url = 'https://www.audible.com/';
    const context = await browser.createIncognitoBrowserContext();
    const page = await auth(context, process.env.jacobCookie);
    try {
        // await page.goto(url);
...
async function auth(context: puppeteer.BrowserContext, cookie: string) {
